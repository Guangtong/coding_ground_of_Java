...
if( process.argv.length < 3 ){
    console.log( "Usage: node index.js port language sessionid projectid");
    return;
}
...
var SERVER = "A";
var WEBSITE = "http://codingground.tutorialspoint.com";
var REDIRECT_URI = "https://codingground.tutorialspoint.com/redirect.php";
var TUTORIALSPOINT = 'http://www.tutorialspoint.com';

if( SERVER === "A" ){
   WEBSITE = 'http://codingground.tutorialspoint.com';
   REDIRECT_URI = "https://codingground.tutorialspoint.com/redirect.php";
}else if( SERVER === "B" ){
   WEBSITE = 'http://codingground1.tutorialspoint.com';
   REDIRECT_URI = "https://codingground1.tutorialspoint.com/redirect.php";
}else if( SERVER === "C" ){
   WEBSITE = 'http://codingground2.tutorialspoint.com';
   REDIRECT_URI = "https://codingground2.tutorialspoint.com/redirect.php";
}else if( SERVER === "D" ){
   WEBSITE = 'http://codingground3.tutorialspoint.com';
   REDIRECT_URI = "https://codingground3.tutorialspoint.com/redirect.php";
}

// Dropbox Keys and secrets
// ID --> contact@tutorialspoint.com
// https://www.dropbox.com/developers/apps
var DROPBOX_APP_KEY;
var DROPBOX_APP_SECRET;
if( SERVER === "A" ){
    // Coding Ground A
   DROPBOX_APP_KEY = 'rbz29yun509xw4u';
   DROPBOX_APP_SECRET = 'tp76mxc6pju5nqa';
}else if( SERVER === "B" ){
    // Coding Ground B
   DROPBOX_APP_KEY = '2bw2layqykrv8zh';
   DROPBOX_APP_SECRET = 'fjyu20nv5a1vnae';
}else if( SERVER === "C" ){
    // Coding Ground C
   DROPBOX_APP_KEY = '6b34hfruu4i0gic';
   DROPBOX_APP_SECRET = 'hpkb8s3btmxp64h';
}else if( SERVER === "D" ){
    // Coding Ground C
   DROPBOX_APP_KEY = 'uo3zhsd6vq8mn53';
   DROPBOX_APP_SECRET = '8c9nwdqlpmi304i';
}

// OneDrive Keys and Secrets
// ID --> codingground@outlook.com
//
var ONEDRIVE_CLIENT_ID = '0000000048131526';
var ONEDRIVE_CLIENT_SECRET = 'NaBSY1SGKPqH1SzZv-DYudzSeCGtzAFy';
...
if( SERVER === "A" ){
    // Coding Ground A
    GOOGLE_CLIENT_ID = '1095649981186-0kd65u02lv9309m9p0jgukafhfc0ukjl.apps.googleusercontent.com';
    GOOGLE_CLIENT_SECRET = 'EVo73UXxA9JtsVWY-hb8Xj6L';
    GOOGLE_SCOPES = 'https://www.googleapis.com/auth/drive';
}else if( SERVER === "B" ){
    // Coding Ground B 
    GOOGLE_CLIENT_ID = '1095649981186-2aepbdrjo9dpdfkk5ogtv28q61ne8ehe.apps.googleusercontent.com';
    GOOGLE_CLIENT_SECRET = 'tY0kiKXLMgjfuam8AGgJET2S';
    GOOGLE_SCOPES = 'https://www.googleapis.com/auth/drive';
}else if( SERVER === "C" ){
    // Coding Ground C 
    GOOGLE_CLIENT_ID = '1095649981186-5ccr1jrij2r4ook6udb0m1vdsh4re96n.apps.googleusercontent.com';
    GOOGLE_CLIENT_SECRET = 'WMD3WWj543hFUr3P5cLDiuBG';
    GOOGLE_SCOPES = 'https://www.googleapis.com/auth/drive';
}else if( SERVER === "D" ){
    // Coding Ground C 
    GOOGLE_CLIENT_ID = '1095649981186-leokuhkflu8515me0aqsf26vqa99a71s.apps.googleusercontent.com';
    GOOGLE_CLIENT_SECRET = 'mKSQoIxtnxiC4PEvLaMqq0bG';
    GOOGLE_SCOPES = 'https://www.googleapis.com/auth/drive';
}
...
if( SERVER === "A" ){
    // Coding Ground A
   GITHUB_CLIENT_ID = '661f57f60655d6cb8a44';
   GITHUB_CLIENT_SECRET = '191d7d4c1ec30906ae0ed47d580fa33a18bba7bb';
}else if( SERVER === "B" ){
   // Coding Ground B
   GITHUB_CLIENT_ID = 'd2092aa5d68f63f1594a';
   GITHUB_CLIENT_SECRET = '7b03c22395914c1d2a24a5b02759495762a7c7c7';
}else if( SERVER === "C" ){
   // Coding Ground C
   GITHUB_CLIENT_ID = '7b46b74a85e7bbca556b';
   GITHUB_CLIENT_SECRET = '241dec7db073a2918c98adcfc555d2e3632dca4e';
}else if( SERVER === "D" ){
   // Coding Ground C
   GITHUB_CLIENT_ID = '571b9d8cbec574e4a8d3';
   GITHUB_CLIENT_SECRET = '9431a3708c329dbba9c6374437fcb57822461691';
}
...
var GOOGLE_WEBCLIENT_SECRET = 'TTULwRR54phV-I1D9U_tfr4u';
var GoogleTokenProvider = require("refresh-token").GoogleTokenProvider;
// Created from https://developers.google.com/oauthplayground/
var GOOGLE_REFRESH_TOKEN = '1/FL2VfN6K_vMNbFQulZZ6JAgpRKkDdgV0MtzPy0TtAm8';

var CODINGGROUND = "codingground";
var port = process.argv[2];
var languageid = process.argv[3];
var cgsessid = process.argv[4];
var projectid = null;
if( process.argv[5] ){
...
function handleDisconnect(){
     ...
     if( ((currentTime - passiveTime ) / 1000 > 60 * 5) && disconnected) { // wait for 5 minutes after disconnected
...
function setLanguage( langid, copyFile ){
    ...
    if( langid === "mysqlterm" || langid === "phpdb" || langid === "perldb" || langid === "pythondb" || langid === "rubydb" || langid === "jdbc" ){
        exec( "/usr/bin/mysqld_safe --datadir=/home/cg/mysql --socket=/home/cg/mysql/mysql.sock --pid-file=/home/cg/mysqld.pid --user=cg&"  + " \n", function (error, stdout, stderr) {
       ...
       if( langid === lang.id ){
           ...
           if( language.command ){
             if( STRING(language.command).contains ("hadoop") || STRING(language.command).contains ("mysql") || STRING(language.command).contains ("redis") || STRING(language.command).contains ("mongo") ){
              ...
              }else if( STRING(language.command).contains ("oracle")){
   ...
   fs.readFile( req.files.file.path, function (err, data) {
        fs.writeFile(file, data, function (err) {
         if( err ){
          ...
          console.log( response );
          // This stupiod is not returning anything
          res.end( JSON.stringify( response ) );
        });
   });
});
...
app.get('/web_view', function(req, res){
   console.log("Got a command " + req.body.command);
   console.log( "Going to issue command : |" + language.preview  + "|");
   if( language.id === 'latex'){
   ...
   exec( language.preview, {cwd:HOME}, function(error, stdout, stderr) {
        var html = '<iframe style="width:100%; height:100%; border:0px;" src="' + WEBSITE + '/user' + port + '/root/index.htm"></iframe>';
        if( language.id === 'latex'){
             fs.exists('/home/cg/root/main.pdf', function (status) {
                  if( !status ){
...
app.post('/send_message', function(req, res){
   ...
   var result = '<html><body><center><img src="http://codingground.tutorialspoint.com/src/images/mini-logo.png"/></center><br><br>' +
   ...
   request.post('http://www.tutorialspoint.com/cgi-bin/tp-contact.cgi', {form:{'contactp':name, 'emailid':mail, 'subject':'Coding Ground' + name, 'message':message}}, function (error, response, body) {
       console.log( response );
       res.end( result );
   });
});
...
app.post('/set_compile_options', function (req, res){
   ...
   result = '<html><body><center><img src="http://codingground.tutorialspoint.com/src/images/mini-logo.png"/></center><br><br>' +
...
io.sockets.on('connection', function(sock) {
  ...
  if( languageid === "oracleterm"){
      if( !STRING(language.command).contains ("oracle")){
          process.exit(0);
      }
  }
...
app.post('/resize', function(req, res){
   ...
   console.log( "Going to resize terminal ");
              ...
              if( w.name === workspace ){
                  ...
                  console.log( "Got work space id " + w.id );
                    ...
                    request.get('https://apis.live.net/v5.0/' + w.id + '/files?filter=folders&access_token=' + token, function (error, response, body) {
                       ...
                       '</head><body><center><img src="http://codingground.tutorialspoint.com/src/images/mini-logo.png"/></center><br>' +
                       '<div style="font-size:24px;text-align:center">' + onedrive_display_name + '</div><br>' +
      ...
      request.get('https://api.github.com/repos/' + github_owner + "/" + workspace + "/contents/", {headers: { 'User-Agent':GITHUB_USER_AGENT, Authorization: 'token ' + token }}, function (error, response, body) {
         ...
         projects.every( function( p ) {
            if( p.type === "dir" ){
               ...
               count = count + 1;
               result = result + '<tr><td>' + count + '</td><td>' + p.name + '</td><td><a href="delete_github_project?state=' + req.query.state  + '&project=' + p.path + '&sha=' + p.sha + '&code=' + req.query.code + '\'">Delete</a></td><td><a href="import_github_project?state=' + req.query.state  + '&project=' + p.path + '&code=' + req.query.code + '\'">Import</a></td></tr>';
