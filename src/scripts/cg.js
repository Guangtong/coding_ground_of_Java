...
$(document).ready(function() {
   ...
   $(window).load(function () {
      ...
      if( terminal <= 0 ){
          //$("#cc").layout('panel','south').panel('resize', {height:fh});
...
*/
                $('#south').css('background-color', '#285627');
                $('#compile').css( "display", "none" );
                ...
                if( languageid === "phpdb" || languageid === "perldb" || languageid === "pythondb" || languageid === "rubydb"){
                    $('#cc').layout('panel', 'east').panel('setTitle', "Result View");
      ...
      var socket = io.connect( HOME + ":" + port);
      socket.on('connect', function() {
      connected = true;
      clearTimeout(timeout);
      socket.emit('verify', {'sessionid':sessionid, 'port':port});
     ...
     // This is webview part
     $("#preview").click(function(){
          if( languageid === "latex" ){
               saveFiles( function( status ){
                     $("#cc").layout('panel','east').panel({
                          href: HOME + ":" + port +  "/web_view",
     ...
     $("#cc").layout('panel','center').panel({
      ...
      setInterval(function(){
         if( resized ){
            ...
            if( document.getElementById('terminal') ){
                 termheight = ( document.getElementById('terminal').clientHeight / term.element.offsetHeight );
                 termwidth = ( document.getElementById('terminal').clientWidth / term.element.offsetWidth );
                 termheight = termheight*term.rows|0, termwidth = termwidth*term.cols|0, term.resize( termwidth, termheight ), typeof func ==='function' && func(termwidth,termheight);
                 term.focus();
            }
            var url = HOME + ':'  + port + '/resize';
            var inputs = {'width': termwidth, 'height': termheight};
            $.ajax({
               type: "POST",
               url: url,
               data: inputs,
      ...
      //   term.emit( 'data', 'figlet -ctf small -w 70 "Coding Ground"' );
      //   term.write('\x1b[1mWelcome to Coding Ground!\x1b[m\r\n');
      //   term.write('\x1b[5m(Use this section to type your Linux commands)\x1b[m\r\n\r\n\r\n');
      setEditorFontSize( editor_font_size );
   ...
   } // If  given port is greater than 0
   });
   ...
   $('#home').tree({
      url : HOME + ':'  + port + '/load_tree?port=' + port + "&sessionid=" + sessionid,
      ...
      onAfterEdit:function(node){
         if(/^[a-zA-Z0-9-_+@.?:=]*$/.test(node.text) == false) {
            ...
            return false;
         }else if( !( node.text.length) && nodetype === "F" ) {
         ...
         var url = HOME + ':'  + port + '/rename_file?port=' + port + "&sessionid=" + sessionid;
         var inputs = { "cwd": cwd, "oldnode": nodetext, "newnode": node.text};
         $.ajax({
            type: "POST",
            url: url,
            data: inputs,
...
function openFileUpload(){
   ...
   $win.window('open');
   $('#sign').window('refresh', HOME + ':'  + port + '/upload-file.htm');
}
function openShareProject(){
   ...
   $win.window('open');
   $('#sign').window('refresh', HOME + ':'  + port + '/share-project.htm');
}
...
function saveAtDropbox(){
   openWindow(  HOME + ':'  + port +"/save_at_dropbox", "Save Project", 700, 500 );
}
function saveAtGithub(){
   openWindow(  HOME + ':'  + port +"/save_at_github", "Save Project", 1050, 500 );
}
function saveAtBox(){
   openWindow(  HOME + ':'  + port +"/save_at_box", "Save Project", 700, 500 );
}
function saveAtGoogleDrive(){
   openWindow(  HOME + ':'  + port +"/save_at_googledrive", "Save Project", 700, 500 );
}
function saveAtOneDrive(){
   openWindow(  HOME + ':'  + port +"/save_at_onedrive", "Save Project", 700, 500 );
}
function listDropboxProjects(){
   openWindow(  HOME + ':'  + port +"/list_dropbox_projects", "Import Project", 700, 500 );
}
function listGithubProjects(){
   openWindow(  HOME + ':'  + port +"/list_github_projects", "Import Project", 1050, 500 );
}
function listBoxProjects(){
   openWindow(  HOME + ':'  + port +"/list_box_projects", "Import Project", 700, 500 );
}
function listOneDriveProjects(){
   openWindow(  HOME + ':'  + port +"/list_onedrive_projects", "Import Project", 700, 500 );
}
function listGoogleDriveProjects(){
   openWindow(  HOME + ':'  + port +"/list_googledrive_projects", "Import Project", 700, 500 );
}
   ...
   $.messager.prompt('Change Project title', 'Enter your project title:' , function(title){
      if (title){
         if(/^[a-zA-Z0-9- ]*$/.test(title) == false) {
            ...
            var url = HOME + ':'  + port + '/rename_project';
            var inputs = {"projecttitle":title};
            $.ajax({
               type: "POST",
               url: url,
               data: inputs,
...
function downloadFile(){
   ...
   var url = HOME + ':'  + port + "/download_file" + "?cwd=" + cwd + "&file=" + filename + '&port=' + port + "&sessionid=" + sessionid;
...
function downloadProject(){
   saveFiles( function( status ){ return true; });
   var url = HOME + ':'  + port + "/download_project" + '?port=' + port + "&sessionid=" + sessionid;
   $('#download').attr('src', url);
}
function reloadTree(){
   ...
   if ($('#codebox').tabs('exists', tabindex)){
      ...
      $('#codebox').tabs('add',{
         ...
         onLoad: function( panel ){
            ...
            editors[tabid].getSession().on('change', function(){ 
         ...
         $('#codebox').tabs('add',{
            ...
            id:tabid,
            closable:true,
            href:HOME + ':'  + port + '/load_file?id=' + tabid,
            extractor:function(data){
            return data;
            },
            onLoad: function( panel ){
...
function refreshTab( cwd, oldfilename, newfilename, mode){
   ...
   if ($('#codebox').tabs('exists', oldtabindex)){
      ...
      // This may not have an editor in case of PDF and images.
      if( editors[oldid] ){
          // Delete old editor.
...
function saveFiles( callback ){
   ...
   for( var key in  editors ){
      ...
      if( !editors[key].getSession().getUndoManager().isClean() ){
         ...
         var url = HOME + ':'  + port + '/save_file';
         var inputs = { "file": filename, "content": content};
...
function loadFile( node, bypass ){
   ...
   var inputs = { "cwd": cwd, "file": filename};
   // Check its mime type first
   var url = HOME + ':'  + port + '/get_mime_type';
   $.ajax({
      type: "POST",
      url: url,
      data: inputs,
...
function deleteFile(){
    ...
    $.messager.defaults.ok = "Yes";
    $.messager.defaults.cancel = "No";
    $.messager.confirm('Confirmation','Do you really want to delete file ' + filetitle, function(r){
       deleting = true;
       if (r){
          // Make Ajax call to delete file
          var url = HOME + ':'  + port + '/delete_file?port=' + port + "&sessionid=" + sessionid;
          var inputs = { "cwd": cwd, "file": filename};
          $.ajax({
               type: "POST",
               url: url,
...
function deleteDir(){
    ...
    $.messager.confirm('Confirmation','Do you really want to delete directory ' + title, function(r){
       if (r){
          // Make Ajax call to delete file
          var url = HOME + ':'  + port + '/delete_dir?port=' + port + "&sessionid=" + sessionid;
          var inputs = { "cwd": cwd, "file": title};
          var retVal = false;
          $.ajax({
               type: "POST",
               url: url,
...
function newFile( ){
   ...
   var url = HOME + ':'  + port + '/add_file?port=' + port + "&sessionid=" + sessionid;
   var inputs = { "cwd": cwd, "file": filename};
   $.ajax({
        type: "POST",
        url: url,
...
function newDir(){
   ...
   var url = HOME + ':'  + port + '/add_dir';
   var inputs = { "cwd": cwd, "dir": dirname};
   $.ajax({
        type: "POST",
        url: url,
        data: inputs,
...
function getProjectTitle(){
     ...
     var url = HOME + ':'  + port + '/get_project_title';
     var inputs = {'hello': 'bye'};
     $.ajax({
      type: "GET",
      url: url,
